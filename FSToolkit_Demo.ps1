...
# Adding to module path
$modulepaths = $env:PSModulePath.Split(';')
if(!($modulepaths -contains $location))
    {
...
Open-SQLiteConnection -ConnectionName dbconnect -DataSource "$location\ROTA.db"
...
[array]$hash.Engg_selected = ($hash.engg_list_box.items | ? {$_.isChecked}).content
...
#<#
$codeblock = {
$URL = 'https://sim.srv.allianz/nav_to.do?uri=%2Fincident_list.do%3Fsysparm_userpref_module%3Dd19ef52315f702002f108c8493a9720b%26sysparm_query%3Dactive%3Dtrue%5Eassignment_groupDYNAMICd6435e965f510100a9ad2572f2b47744%5EstateNOT%20IN6,7%5EGROUPBYassignment_group%26sysparm_clear_stack%3Dtrue'
...
while($hash.flag)
    ...
    foreach($line in $parsedcontent)
        {
        $line -match '(?<incident>^INC[0-9]+)\s+?(?<assigned_to>.+)?(?<queue>AZTEC.+?)\s' | Out-Null
    ...
    foreach($record in $unassigned)
        {
        Add-Content -Path $verboselogs -Value "Processing $record"
        ...
        if($pEng)
            ...
            if($inci_q.Value[$index] -eq $username)
                ...
                Add-Content -Path $verboselogs -Value "$(Time-Stamp) -- Next in queue is $($inci_q.Value[($index)])"
                continue
                }
            else{
                $assigned_to_field = ''
...
#>
$hash.nullQ = ''
$hash.nullQ = $hash.qs | ?{$_.value.count -eq 0}
if(!$hash.nullQ)
    {
    Write-Host 'Runspace started'
    #<#
    # Create Runspace
    ...
    {
    [System.Windows.MessageBox]::Show("No engineer assigned to $($hash.nullQ.name.TrimEnd('_Q'))")
    }
})
